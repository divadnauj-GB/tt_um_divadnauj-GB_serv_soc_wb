...
# RISC-V SoC based on the SERV core
...
![alt text](./docs/serv-e-wb-soc.svg)
...
## Contact
Connect with me on [LinkedIn](https://www.linkedin.com/in/juan-david-guerrero-balaguera/).

## Troubleshoting 16bit data types
Recently I discovered that the SoC might fail when using 16bit data types. It was a rookie mistake, After countless hours of debugging, I found out that the SH intruction (Store half word) uses big endian instead of little endian. The problem was in the qspi memory controller when handling 16bit stores. I already patched the RTL so when using FPGAs the problem is solved, Unfortunatelly on the ASIC submitted to the **ttgf26a** shuttle already has the issue. Tehrefore, we need to avoid using SH intructions direclty on the hardware. There might be sophisticated solutions to patch the ASIC problem such as modifying the compiler or creating an specific SDK for the SERV-SoC. However, the problem might be just for that ASIC so I'm not fully convinced that it worth it to make such an effort. you can simply avoid using 16bit datatypes. Therefore, I just decided to write a simple python script able to patch the final ELF file using some trampolin instructions. It is not an elegant solution, but it does its job, at least for small programs. 
